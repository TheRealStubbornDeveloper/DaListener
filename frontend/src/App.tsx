...
import type { DashboardEvent, IntelligenceNotes, Meeting, OpenAIStatus, TranscriptEvent } from "./types";
...
export default function App() {
  ...
  async function saveKey() {
    try { setOpenAI(await api.saveOpenAIKey(apiKey) as OpenAIStatus); setApiKey(""); setNotice("OpenAI key stored in Windows Credential Manager."); }
    catch (reason) { setError(String(reason)); }
  }
  ...
  return <>
    <header className="topbar"><div><h1>DaListener Meeting Copilot</h1><p>OpenAI-powered, independent live transcription for Chromium meeting tabs.</p></div><div className="header-actions"><span className={`connection ${connection}`}>{connection}</span><button onClick={() => api.openTranscriptFolder()}>Open transcripts</button><button onClick={copyPairing} disabled={!openai?.configured}>Pair extension</button></div></header>
    <main className="page">
      {error && <div className="error-banner">{error}</div>}{notice && <div className="pairing">{notice}</div>}
      {openai && !openai.configured && <section className="panel setup"><span className="eyebrow">ONE-TIME SETUP</span><h2>Connect OpenAI</h2><p>The key stays in the local Python bridge and is stored by Windows Credential Manager. It is never sent to this dashboard or the extension again.</p><div className="key-row"><input type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder="sk-…" /><button onClick={saveKey} disabled={apiKey.length < 20}>Save securely</button></div></section>}
      <section><div className="section-heading"><div><span className="eyebrow">MEETINGS</span><h2>Captured browser tabs</h2></div>{openai && <div className={openai.configured ? "capacity" : "capacity warning"}>{openai.active_streams} active · {openai.transcription_model} · {openai.status}</div>}</div>
        <div className="meeting-grid">{meetings.map(meeting => <button className={`meeting-card ${selected?.id === meeting.id ? "active" : ""}`} key={meeting.id} onClick={() => setSelectedId(meeting.id)}><div><span className="eyebrow">{meeting.browser} tab {meeting.tab_id ?? "—"}</span><span className={`badge ${meeting.status}`}>{meeting.status}</span></div><h3>{meeting.title}</h3><p>OpenAI · {meeting.transcription_model}</p>{meeting.last_error && <p className="error-text">{meeting.last_error}</p>}</button>)}{!meetings.length && <div className="empty-card"><h3>No captured meetings</h3><p>Configure OpenAI, pair the extension, then click its icon in each meeting tab.</p></div>}</div>
      </section>
